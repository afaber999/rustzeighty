...
mod test_zex {
    ...
    fn cpm_bdos(cpu: &mut rz80::CPU, mem : &mut rz80::Memory) {
        match cpu.reg.c() {
            2 => {
                // output a character
    ...
    fn run_test(prog: &[u8]) -> (i64, i64) {
        ...
        loop {
            num_ops += 1;
            num_cycles += cpu.step(&mut bus, &mut mem);
            match cpu.reg.pc() {
                0x0005 => { cpm_bdos(&mut cpu, &mut mem); },  // emulated CP/M BDOS call
                0x0000 => { break; },
                _ => { },
            }
        }
