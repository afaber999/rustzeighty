...
if .debug > 0
	call	iputs
	db	0x0D,0x0A,".bios_boot entered", 0x0D,0x0A,0
	call	iputs
...
.boot_msg:
	defb	0x0D, 0x0A, 0x0D, 0x0A
	defb	'Rustzeighty BIOS Copyright (C) 2022 A.L. Faber', 0x0D, 0x0A
	defb	'CP/M 2.2 Copyright (C) 1979 Digital Research', 0x0D, 0x0A
	defb	'  git: @@GIT_VERSION@@', 0x0D, 0x0A
	defb	'build: @@DATE@@', 0x0D, 0x0A, 0x0D, 0x0A, 0x0
...
if .debug >= 2
	call	iputs
	db	0x0D, 0x0A, ".bios_wboot entered", 0x0D,0x0A,0
endif
...
.wboot_loop:
	...
	call	iputs
	db      0x0D,0x0A,0x0A,"*** ERROR: WBOOT READ FAILED.  HALTING."
	db      0x0D,0x0A,0x0A,"*** PRESS RESET TO REBOOT ***", 0x0D,0x0A, 0
...
.wboot_sec:
	...
	ld	a,b
	or	c
	jp	NZ, .wboot_loop ; if BC != 0 then goto .wboot_loop

if .debug >= 0
	call	iputs
	db	0x0D, 0x0A, "$$$$$ -> SECTORS COPIED START CPM AGAIN .bios_wboot entered", 0x0D,0x0A,0
endif

if .debug >= 5
	; dump the first part that's been read from block device
	ld	hl,0xD400		; start address
	ld	bc,0x400	; number of bytes
	ld	e,1		; fancy format
	call	hexdump
endif
...
if .debug >= 5
	; dump first sectors read from block device
	ld	hl,0xD400		; start address
	ld	bc,0x400	; number of bytes
	ld	e,1		; fancy format
	call	hexdump
...
if .debug >= 2
	call	iputs
	db	".bios_home entered", 0x0D,0x0A,0x00
endif
...
if .debug >= 2
	call	iputs
	db	".bios_read cache miss:", 0x0D, 0x0A, 0
	call	.debug_disk
endif
...
if .debug >= 4
	call	iputs
	db	"$$$$$ BLK BUFFER bios_sdbuf $$$$$$$ ",0x0D,0x0A,0

	; dump the sector for reference
	ld	hl,.bios_sdbuf		; start address
	ld	bc,0x0200	; number of bytes
	ld	e,1		; fancy format
	call	hexdump
endif 

    ld a, 0
	or	a			; was the blk driver read OK?
	jr	z,.bios_read_sd_ok

	call	iputs
	db	"BIOS_READ FAILED!", 0x0D, 0x0A, 0
...
if .debug >= 2
	call	iputs
	db	0x0D,0x0A,".bios_write cache miss: ",0
	call	.debug_disk
endif

    ; turn on tracing
    ;call dbg_set_trace_on
...
.rd_blk_wbt:    
    ...
    ; AF FIX ERROR HANDLING
    ld a,0    
	or	a			; was the SD driver read OK?
	jr	z,.bios_write_sdbuf

	call	iputs
	db	"BIOS_WRITE SD CARD PRE-READ FAILED!", 0x0D, 0x0A, 0
	ld	a,1			; tell CP/M the read failed
	ld	(.bios_sdbuf_val),a	; mark the .bios_sdbuf as invalid
	jp	.bios_write_ret
...
.bios_write_sdbuf:
	...
	ld	de,.sd_partition_base	; XXX add the starting partition block number
	add	hl,de			; HL = SD physical block number
	; set active block which will write to
    ld a,0
    out (blk_dev_blk_b3),a
    out (blk_dev_blk_b2),a
    ld a,h
    ...
    ; copy all 512 bytes from buffer into block device
    ld de, 0x0200
    ld hl,.bios_sdbuf

.wr_blk_char:
    ld  a,(hl)
    out (blk_dev_wr_char), a
    inc hl
    dec de
    ld a,d
    or a,e
    ...
    ; write to device
    ld a,0
    out (blk_dev_wr_blk),a

	or	a
	jr	z,.bios_write_ret

	call	iputs
	db	"BIOS_WRITE SD CARD WRITE FAILED!", 0x0D, 0x0A, 0
	ld	a,1			; tell CP/M the read failed
	ld	(.bios_sdbuf_val),a	; mark the .bios_sdbuf as invalid
...
.bios_alv_a:
	dc	(4087/8)+1,0xaa	; scratchpad used by BDOS for disk allocation info
.bios_alv_a_end:


.bios_sdbuf_trk:		; The CP/M track number last left in the .bios_sdbuf
	dc	2,0xff		; initial value = garbage
.bios_sdbuf_val:		; The CP/M track number in .bios_sdbuf_trk is valid when this is 0
	dc	1,0xff		; initial value = INVALID
.bios_sdbuf:			; scratch area to use for SD block reading and writing
	dc	512,0xa5        ; initial value = garbage


.bios_stack_lo:
	dc	64,0x55		; 32 stack levels = 64 bytes (init to analyze)
